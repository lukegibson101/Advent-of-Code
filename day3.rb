rucksacks = "FzQrhQpJtJMFzlpplrTWjTnTTrjVsVvvTnTs
...
list = rucksacks.split(/\n/)
sum=0
for i in 0..list.count-1
    length = list[i].length/2
    split = list[i].chars.each_slice(length).map(&:join)
    for i in 0..1
      split[i] = split[i].split("")
    end
    hash = Hash[('a'..'z').to_a.zip((1..26).to_a)+('A'..'Z').to_a.zip((27..52).to_a)]
    split = (split[0]&split[1]).map{|char| hash[char]}
...
for i in 0..list.count-1
    ...
    if x < 3
      ...
      x=1
      hash = Hash[('a'..'z').to_a.zip((1..26).to_a)+('A'..'Z').to_a.zip((27..52).to_a)]
      group = (group[0]&group[1]&group[2]).map{|char| hash[char]}
      sum += group[0]
      group = [] 
    end
end  
